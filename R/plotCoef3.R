plotCoef3 <- function(beta,ind,norm,lambda,df,dev,bic,label=FALSE,xvar=c("norm","lambda","dev"),xlab=iname,ylab="Coefficients",...){
    ...
    nzerorow = apply(abs(beta) > 1e-5/max(abs(beta)), 1, sum)
    beta = beta[order(-nzerorow),]
    xvar=match.arg(xvar)
    switch(xvar,
    "norm"={
