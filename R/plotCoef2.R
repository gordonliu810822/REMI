plotCoef2 <- function(beta,ind,norm,lambda,df,dev,bic,label=FALSE,xvar=c("norm","lambda","dev"),xlab=iname,ylab="Coefficients",...){
    ...
    type=dotlist$type
    if(is.null(type)){
        plot(1,type="n",xlim = c(min(index),max(index)),ylim=c(min(beta),max(beta)),xlab=xlab,ylab=ylab,...)
        for ( i in 1:nwhich){
            if (which[i] %in% ind)
            colr = "red"
            else colr = "blue"
            lines(index,beta[i,],type="l",lty=1,xlab=xlab,ylab=ylab,col=colr)
        }
    }
    if(!is.null(type)){
        plot(1,type="n",xlim = c(min(index),max(index)),ylim=c(min(beta),max(beta)),xlab=xlab,ylab=ylab,...)
        for ( i in 1:dim(beta)[1]){
            ...
            lines(beta[i,],lty=1,xlab=xlab,ylab=ylab,col=colr)
        }
    }
    ...
    if (is.null(bic) == F){
        indx.min = which(bic == min(bic));
        lambda.min = lambda[indx.min];
